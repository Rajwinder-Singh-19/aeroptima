...
from database.PCA_aerofoils import PCAFoil as UDB

DATASET_PATH = os.getcwd() + "/src/database/PCA_bezier_controls.npy"
N_SEGMENTS = 10
...
def process_airfoil(aerofoil_name):
    ...
    try:
        bezfoil = BezierFoil(UDB[aerofoil_name], n_segments=N_SEGMENTS)
...
if __name__ == "__main__":
    ...
    num_workers = min(4, 6)  # Adjust this based on your system

    with Pool(num_workers) as pool:
        dataset = pool.map(process_airfoil, UDB.keys())
